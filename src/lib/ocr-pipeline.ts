...
import { getIndex, setIndexEntry, setPage, getPage, computeRawText, rebuildSearchIndex, type OcrIndexEntry } from "./ocr-store";
...
export async function resumeIfNeeded() {
  ...
  const processingDocs = Object.entries(index).filter(
    ([, e]) => e.status === "processing" && e.manifestUrl
  );
  if (processingDocs.length === 0) return;

  // Also pick up any queued/pending docs while we're at it
  const toProcess = Object.entries(index).filter(
    ([, e]) => (e.status === "processing" || e.status === "queued" || e.status === "pending") && e.manifestUrl
  );
