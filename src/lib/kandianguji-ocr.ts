...
import { recordCall } from "./ocr-usage";
...
export async function callKandianguji(
  imageBase64: string,
  options?: { detMode?: string; detLayout?: boolean; skipUsageTracking?: boolean }
  ...
  if (ocrData.message !== "success" || !ocrData.data) {
    if (!options?.skipUsageTracking) {
      await recordCall({ success: false }).catch(() => {});
    }
    throw new Error(
  ...
  if (!options?.skipUsageTracking) {
    await recordCall({ success: true }).catch(() => {});
  }
...
/**
 * Convert Kandianguji response data into SpatialCharacter[] format.
 */
