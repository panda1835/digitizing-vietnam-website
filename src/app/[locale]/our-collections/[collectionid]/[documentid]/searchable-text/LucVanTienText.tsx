...
export default function LucVanTienText() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const res = await fetch(`/api/luc-van-tien`);
        ...
        <div className="flex flex-col md:flex-row md:justify-center space-x-4">
          ...
          <div>
            {textData[pageIndex] && textData[pageIndex].$ && (
              <Image
                unoptimized
                src={`https://backend.digitizingvietnam.com/images/iiif/2/van-tien-co-tich-tan-truyen/${textData[pageIndex].$.pi}/full/full/0/default.jpg`}
                alt="Truyen Kieu"
                width={200}
                height={300}
