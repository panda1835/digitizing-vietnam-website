...
export async function generateMetadata({
  ...
  try {
    ...
    if (data.data && data.data.length > 0) {
      ...
      const description = stripHtmlTags(item.description || item.content || "");
    ...
    console.error("Error fetching outreach item metadata:", error);
  }
...
import { Outreach } from "../../page";

interface OutreachWithCollection extends Outreach {
  pedagogy_collection?: {
    slug: string;
    title: string;
  };
...
const OutreachResource = async ({
  params,
}: {
  params: { locale: string; collectionSlug: string; itemSlug: string };
}) => {
  const { locale, itemSlug } = params;
  const t = await getTranslations();

  let post: OutreachWithCollection = {
    title: "",
    description: "",
    contributors: [""],
    content: "",
  ...
  try {
    ...
    console.error("Error fetching outreach item:", error);
  }
  ...
  if (post.pedagogy_collection) {
    breadcrumbItems.push({
      ...
      href: `outreach/${post.pedagogy_collection.slug}`,
    });
  }
...
export default OutreachResource;
