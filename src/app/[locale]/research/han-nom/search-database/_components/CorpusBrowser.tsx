...
export default function CorpusBrowser() {
    ...
    const [activeTab, setActiveTab] = React.useState<"library" | "results">("library");
    ...
    const handleSearch = async (e: React.FormEvent) => {
        ...
        setIsSearching(true);
        try {
            const response = await fetch(`/api/research/han-nom/search-database?q=${encodeURIComponent(searchQuery)}`);
            const data = await response.json();
    ...
    const languages = React.useMemo(() => {
        const set = new Set(works.map((w) => w.language).filter(Boolean) as string[]);
        return Array.from(set).sort();
    }, [works]);

    const filtersActive =
        titleQuery !== "" || filterGenre !== "all" || filterLanguage !== "all" || filterCuration !== "all" || yearFrom !== "" || yearTo !== "" || contributorQuery !== "";
    ...
    const displayedWorks = React.useMemo(() => {
        let list = works.map((w, i) => ({ ...w, _idx: i })).filter((w) => {
            if (titleQuery.trim() !== "" && !w.title.toLowerCase().includes(titleQuery.toLowerCase())) return false;
            if (filterGenre !== "all" && w.genre !== filterGenre) return false;
            if (filterLanguage !== "all" && w.language !== filterLanguage) return false;
            if (filterCuration !== "all" && (w.curationStatus ?? "wiki") !== filterCuration) return false;
            if (yearFrom !== "") {
            ...
            if (contributorQuery.trim() !== "") {
                ...
                const match = w.attributions?.some((a) => a.name.toLowerCase().includes(q));
        ...
        return list;
    }, [works, titleQuery, filterGenre, filterLanguage, filterCuration, yearFrom, yearTo, contributorQuery, sortKey, sortDir]);
    ...
    const groupedSearchResults = React.useMemo(() => {
        const groups: Record<string, { work: string; slug: string; externalPath?: string; results: SearchResult[] }> = {};
        ...
        return Object.values(groups).sort((a, b) => b.results.length - a.results.length);
    }, [searchResults]);
    ...
    return (
        <div className="w-full max-w-5xl mx-auto px-4 py-10">
            {/* Page header */}
            <div className="mb-8 border-b border-branding-brown/20 dark:border-white/10 pb-4">
                <h1 className="text-2xl font-merriweather font-bold text-branding-black dark:text-zinc-100 mb-1">
                    Hán-Nôm Corpus Database
                </h1>
                <p className="text-sm text-muted-foreground dark:text-zinc-400">
                    A digital library of classical Vietnamese literature and historical documents.
                </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                ...
                <div className="bg-branding-gray/5 dark:bg-zinc-800/30 p-4 rounded-lg border border-branding-brown/10 dark:border-white/5">
                    ...
                    <form onSubmit={handleSearch} className="flex gap-2">
                        <div className="relative flex-grow">
                            ...
                            <Input
                                placeholder="Search all texts for terms…"
                    ...
                    <p className="text-[10px] text-muted-foreground mt-2 italic">
                        Scans the entire database for matching Hán-Nôm or Quốc ngữ excerpts.
                    </p>
                </div>
                ...
                <div className="bg-branding-gray/5 dark:bg-zinc-800/30 p-4 rounded-lg border border-branding-brown/10 dark:border-white/5">
                    ...
                    <div className="relative">
                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground dark:text-zinc-400" />
                        <Input
                            placeholder="Find a text by title…"
                            className="pl-9 h-10 text-sm border-branding-brown/20 dark:border-white/10 focus:border-branding-brown/50 dark:bg-zinc-800 dark:text-zinc-100 dark:placeholder:text-zinc-500"
                            value={titleQuery}
                            onChange={(e) => {
                    ...
                    <p className="text-[10px] text-muted-foreground mt-2 italic">
                        Filters the library list below in real-time.
                    </p>
                </div>
            </div>

            {/* Filter bar */}
            <div className="flex flex-wrap items-center gap-2 mb-5 bg-white dark:bg-zinc-900 sticky top-0 z-10 py-2 border-b border-transparent shadow-sm shadow-black/0">
                ...
                <div className="flex items-center gap-1">
                    <span className="text-xs text-muted-foreground dark:text-zinc-400">Year</span>
                    <Input
                        type="number"
                        placeholder="From"
                    ...
                    />
                    <span className="text-xs text-muted-foreground dark:text-zinc-400">–</span>
                    <Input
                        type="number"
                        placeholder="To"
                        className="h-8 w-20 text-xs border-branding-brown/20 dark:border-white/10 dark:bg-zinc-800 dark:text-zinc-200"
                ...
                <span className="ml-auto text-xs text-muted-foreground dark:text-zinc-500">
                    {!isLoading && `${displayedWorks.length} of ${works.length} works`}
                </span>
            </div>
            ...
            <div className="flex gap-0 border-b border-branding-brown/20 dark:border-white/10 mb-4 text-sm">
                <button
                    ...
                    className={`px-4 py-2 border-b-2 transition-colors font-medium ${activeTab === "library"
                ...
                <button
                    ...
                    className={`px-4 py-2 border-b-2 transition-colors font-medium disabled:opacity-40 ${activeTab === "results"
                        ? "border-branding-brown text-branding-brown dark:text-branding-orange dark:border-branding-orange"
                        : "border-transparent text-muted-foreground hover:text-branding-black dark:hover:text-zinc-100"
                        }`}
                >
                ...
                <div className="text-sm">
                    <table className="w-full border-collapse">
                        <thead>
                            <tr className="text-xs uppercase tracking-wide text-muted-foreground dark:text-zinc-400 bg-branding-gray/5 dark:bg-zinc-800/50 border border-branding-brown/10 dark:border-white/10">
                                ...
                                <th className="px-3 py-2 font-semibold text-left">
                                    <button className="flex items-center gap-1 hover:text-branding-black dark:hover:text-zinc-100 transition-colors" onClick={() => handleSort("title")}>
                                        Title <SortIcon col="title" />
                                    </button>
                                </th>
                                <th className="px-3 py-2 font-semibold text-left w-[90px]">Genre</th>
                                <th className="px-3 py-2 font-semibold text-left w-[100px]">Language</th>
                                <th className="px-3 py-2 font-semibold text-left">Contributors</th>
                                <th className="px-3 py-2 font-semibold text-left w-[110px]">
                                    <button className="flex items-center gap-1 hover:text-branding-black dark:hover:text-zinc-100 transition-colors" onClick={() => handleSort("year")}>
                                        Year <SortIcon col="year" />
                                    </button>
                                </th>
                                <th className="px-3 py-2 font-semibold text-left w-[80px]">Status</th>
                                <th className="px-3 py-2 font-semibold text-right w-[80px]">
                                    <button className="flex items-center gap-1 ml-auto hover:text-branding-black dark:hover:text-zinc-100 transition-colors" onClick={() => handleSort("pages")}>
                                        Pages <SortIcon col="pages" />
                                    </button>
                                </th>
                                <th className="w-8"></th>
                            </tr>
                        ...
                        <tbody>
                            {isLoading ? (
                                [1, 2, 3, 4, 5].map((i) => (
                                    <tr key={i} className="border-b border-branding-brown/8 dark:border-white/5 animate-pulse">
                                        <td className="px-3 py-2.5"><div className="h-3 w-4 bg-branding-gray/20 dark:bg-zinc-700 rounded" /></td>
                                        <td className="px-3 py-2.5"><div className="h-3 w-48 bg-branding-gray/20 dark:bg-zinc-700 rounded" /></td>
                                        <td className="px-3 py-2.5"><div className="h-3 w-14 bg-branding-gray/20 dark:bg-zinc-700 rounded" /></td>
                                        <td className="px-3 py-2.5"><div className="h-3 w-12 bg-branding-gray/20 dark:bg-zinc-700 rounded" /></td>
                                        <td className="px-3 py-2.5"><div className="h-3 w-24 bg-branding-gray/20 dark:bg-zinc-700 rounded" /></td>
                                        <td className="px-3 py-2.5"><div className="h-3 w-10 bg-branding-gray/20 dark:bg-zinc-700 rounded" /></td>
                                        <td className="px-3 py-2.5 text-right"><div className="h-3 w-8 bg-branding-gray/20 dark:bg-zinc-700 rounded ml-auto" /></td>
                                        <td className="px-3 py-2.5"><div className="h-3 w-16 bg-branding-gray/20 dark:bg-zinc-700 rounded" /></td>
                                        <td></td>
                                    </tr>
                                ))
                            ) : displayedWorks.length === 0 ? (
                                <tr>
                                    <td colSpan={8} className="text-center py-12 text-muted-foreground dark:text-zinc-500">
                                    ...
                                    <tr
                                        ...
                                        onClick={() => {
                                            if (work.externalPath) {
                                                ...
                                                router.push(`/research/han-nom/search-database/${work.slug}`);
                                            }
                                        }}
                                    >
                                        ...
                                        <td className="px-3 py-2.5">
                                            {work.genre ? (
                                                <Badge variant="secondary" className="text-xs bg-branding-brown/8 text-branding-brown dark:bg-white/10 dark:text-zinc-300 border-none font-normal">
                                                    {work.genre}
                                                </Badge>
                                            ) : (
                                        ...
                                        <td className="px-3 py-2.5">
                                            {work.language ? (
                                                <Badge variant="secondary" className="text-xs bg-branding-orange/8 text-branding-orange dark:bg-branding-orange/20 dark:text-orange-300 border-none font-normal">
                                                    {work.language}
                                                </Badge>
                                            ) : (
                                                <span className="text-muted-foreground text-xs italic">—</span>
                                                ...
                                                <div className="flex flex-col gap-0.5">
                                                    {work.attributions.map((a, ai) => (
                                                        <span key={ai} className="text-xs">
                                                            <span className="text-branding-black dark:text-zinc-200">{a.name}</span>
                                                            <span className="text-muted-foreground dark:text-zinc-500 ml-1">({a.role})</span>
                                                        </span>
                                                    ))}
                                                </div>
                                            ) : (
                                        ...
                                        <td className="px-3 py-2.5 text-muted-foreground dark:text-zinc-500 tabular-nums">
                                            {work.year ?? <span className="italic opacity-50 text-xs">Unknown</span>}
                                        </td>
                                        ...
                                        <td className="px-3 py-2.5">
                                            {work.curationStatus && (
                                                <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${work.curationStatus === "curated"
                                                    ? "bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-400"
                                                    : "bg-yellow-100 text-yellow-700 dark:bg-yellow-900/40 dark:text-yellow-400"
                                                    }`}>
                                                    {work.curationStatus === "curated" ? "Curated" : "Wiki"}
                                                </span>
                                            )}
                                        </td>
            ...
            {activeTab === "results" && (
                <div className="space-y-4">
                    {groupedSearchResults.map((group) => {
                        ...
                        return (
                            <div key={group.slug} className="border border-branding-brown/10 dark:border-white/10 rounded-lg overflow-hidden bg-white dark:bg-zinc-900 shadow-sm">
                                {/* Group Header */}
                                <button
                                ...
                                >
                                    <div className="flex items-center gap-3">
                                        <div className={`p-1 rounded-md bg-branding-brown/10 dark:bg-branding-orange/10 ${isExpanded ? "rotate-0" : "-rotate-90"} transition-transform duration-200`}>
                                    ...
                                    <Badge className="bg-branding-brown/10 text-branding-brown dark:bg-white/10 dark:text-zinc-300 border-none tabular-nums font-semibold">
                                        {group.results.length} hit{group.results.length !== 1 ? "s" : ""}
                                    </Badge>
                                </button>
                                        ...
                                        <table className="w-full border-collapse text-sm">
                                            <thead>
                                                <tr className="text-[10px] uppercase tracking-wider text-muted-foreground dark:text-zinc-400 bg-branding-gray/5 dark:bg-zinc-800/30">
                                                    <th className="text-left px-4 py-2 font-semibold w-[180px]">Location</th>
                                                    <th className="text-left px-4 py-2 font-semibold w-[100px]">Script</th>
                                                    <th className="text-left px-4 py-2 font-semibold">Excerpt</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                    ...
                                                    <tr
                                                        ...
                                                        onClick={() => {
                                                            if (result.externalPath) {
                                                                let path = result.externalPath;
                                                                const params = new URLSearchParams();
                                                                if (result.page) params.set('page', result.page.toString());
                                                                if (result.book) params.set('book', result.book);
                                                                if (result.topic) params.set('topic', result.topic.toString());
                                                                if (result.line) params.set('line', result.line);
                                                                if (searchQuery) params.set('q', searchQuery);

                                                                const queryString = params.toString();
                                                                const finalUrl = path + (queryString ? (path.includes('?') ? '&' : '?') + queryString : '');
                                                                window.open(finalUrl, '_blank');
                                                            } else {
                                                                window.open(`/research/han-nom/search-database/${result.slug}${result.page ? `?page=${result.page}` : ''}`, '_blank');
                                                            }
                                                        }}
                                                    >
                                                        ...
                                                        <td className="px-4 py-3">
                                                            <Badge
                                                                variant="outline"
                                                                className={`text-[9px] px-1.5 uppercase font-medium tracking-normal h-4 ${result.type === "qn"
                                                            ...
                                                            >
                                                                {result.type === "han" ? "Hán" : result.type === "nom" ? "Nôm" : "QN"}
                                                            </Badge>
                                                        </td>
                                                        <td className="px-4 py-3 font-merriweather text-zinc-800 dark:text-zinc-200 leading-relaxed italic border-l-2 border-transparent group-hover:border-branding-brown dark:group-hover:border-branding-orange transition-all">
                                                            <span className="" dangerouslySetInnerHTML={{ __html: `&ldquo;${result.text}&rdquo;` }} />
                                                        </td>
                                                    </tr>
                                                ))}
                                            </tbody>
                    ...
                    <div className="flex justify-between items-center px-2">
                        <p className="text-xs text-muted-foreground dark:text-zinc-500 italic">
                            {groupedSearchResults.length} book{groupedSearchResults.length !== 1 ? "s" : ""} found containing &ldquo;{searchQuery}&rdquo;
                        </p>
                        <p className="text-xs text-muted-foreground dark:text-zinc-500 font-medium bg-branding-gray/10 dark:bg-white/5 py-1 px-3 rounded-full">
                            Total {searchResults.length} match{searchResults.length !== 1 ? "es" : ""}
                        </p>
                    </div>
                </div>
            )}
