...
export default function CompactRadicals({
  ...
  const filteredRadicals = filterChar.trim()
    ? radicals.filter((radical) => relatedChars.has(radical.hn))
    : radicals;
      ...
      <div className="flex-1 min-h-0 flex flex-col">
        <div className="flex items-center mb-2 shrink-0 gap-2">
          ...
          <input
            type="text"
            value={filterChar}
            onChange={(event) => setFilterChar(event.target.value)}
        ...
        <div className="border rounded-lg p-3 flex-1 min-h-0 overflow-y-auto">
          {filterChar.trim() ? (
            filterLoading ? null : filteredRadicals.length === 0 ? (
