...
export default function Entry({
  ...
  const queryChars = componentMatches.length > 0
    ...
    <div className="space-y-8">
      {componentMatches.length > 0 && (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          <div className={`text-lg text-branding-brown mb-3 ${merriweather.className}`}>
            {locale === "en"
              ? `Characters with components ${queryChars.map((ch) => `[${ch}]`).join("")}`
              : `Chữ có các thành phần ${queryChars.map((ch) => `[${ch}]`).join("")}`}
          </div>
          <div className="flex flex-wrap gap-4 text-3xl" style={nomNaTongStyle}>
            {componentMatches.map((ch) => (
              <Link
                key={ch}
                href={`/tools/han-nom-dictionaries/general?q=${encodeURIComponent(ch)}`}
